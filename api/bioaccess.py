...
set_post_lost_users = namespace.inherit("post_lost_users",
    {
        "lost_users_file": fields.String(
            description="Excel file with all user info lost (get this file with stimate factor API), see the example (ensure that IP adress is used instead of alias", 
            example="Registros_Perdidos.xlsx", 
            required=True
            ),
        "devices_info_file": fields.String(description="Excel file with device information, MAC, Alis, IP_Address, Access_Code, and hub_name_or_IP ", 
            example= "dispositivos.xlsx", 
            required=True),
...
class PostService:
    ...
    def requestToken(self):
        ...
        try:
            ...
            r = requests.post(url=tokenURL, json=self.tokenInfo, timeout=REQUEST_TIMEOUT,verify=False)  # add verify False SSL
        except Exception as e:
...
class ClassPostLostUsers(Resource):
    ...
    def post_lost_users_from_files(self,lost_users_file,devices_info_file,usuarios_info_file):
        ...
        devices=[]
        try:
            pd_users = pd.read_excel(lost_users_file)
            pd_devices_info = pd.read_excel(devices_info_file)
            pd_usuarios_info = pd.read_excel(usuarios_info_file)
            result = {'result':'files loaded'}
            devices = pd_users.device.unique()
        except Exception as e:
            result = {'result':'Exception was found:' + str(e)}
        results.append(result)

        result = {'action':'Collect lost user information'}
        results.append(result)
        result = {'result':'No user information was found'}
        for device in devices:
            ...
            if (len(userList)>0):
                ...
                try:
                    ...
                    result = {'result':'Post user list OK'}
                except Exception as e:
                    result = {'result':'Exception was found:' + str(e)}
                results.append(result)
    ...
    @namespace.expect(set_post_lost_users, validate=True)
...
class ClassSetMaintenance(Resource):
    ...
    def post(self):
        ...
        respuesta={}
        source_file_path = 'api/screen_mantenimiento.jpg'
...
class ClassGetScreenshot(Resource):
    ...
    def post(self):
        ...
        if (image_added):
            ...
            now = datetime.datetime.now()
            collage_image_name = str(date.today()) + '_' + str(now.strftime("%H_%M_%S")) + '_screenshots.jpg'
